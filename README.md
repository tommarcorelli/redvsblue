...
- **43 scénarios attaque/défense** couvrant 6 grandes familles techniques :
  élévation de privilèges Linux, réseau & annuaires, conteneurs &
  orchestration, cloud & infrastructure as code (dont identité & secrets),
  applications web (dont IDOR, mass assignment, exposition excessive de
  données et absence de limitation de débit côté API), et Active Directory
  / Windows (AS-REP Roasting, délégation Kerberos, DCSync, GPO).
...
js/
  scenarios.js         Les 43 scénarios (vfs, règles d'exploit, correctifs)
