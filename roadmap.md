...
### v3.9 — Pack « Active Directory », 3ᵉ vague — Silver Ticket & abus d'ACL GenericAll 🔴
**Statut : fait** (2 scénarios supplémentaires, portant le total à 59)
- **Silver Ticket** : à partir du seul hash NTLM d'un compte de service, un ticket de service peut être forgé localement pour un SPN précis — sans jamais obtenir de TGT ni contacter le contrôleur de domaine, donc sans rien y journaliser. Un complément direct du Kerberoasting déjà présent (v3.5) : l'un obtient le hash en le cassant, l'autre suppose le hash déjà connu et montre ce qu'on peut en faire sans jamais revenir vers le DC
- **Abus d'ACL GenericAll** : une délégation ancienne jamais révoquée accorde le droit `GenericAll` à un groupe de support sur le compte d'un administrateur du domaine — un droit qui inclut la réinitialisation de mot de passe. Aucune technique d'exploitation sophistiquée ici : la faille est l'ACL elle-même, ce qui la rend particulièrement facile à manquer lors d'un audit qui ne regarde que les groupes et pas les ACE object-level
- Ces deux scénarios rejoignent la famille « Active Directory / Windows », qui passe ainsi de 7 à 9 scénarios
- Aucune nouvelle famille technique nécessaire ; le mentor contextuel et le Bilan de compétences héritent automatiquement des deux nouveaux scénarios via la famille existante

### v4.0 — Pack « cloud », 2ᵉ vague — chaîne d'approvisionnement Terraform & politique de ressource publique 🟢
**Statut : fait** (2 scénarios supplémentaires, portant le total à 61)
- **Module Terraform non épinglé** : un module référencé par une branche mouvante (`?ref=main`) plutôt qu'un commit épinglé permet, en poussant du contenu malveillant sur cette branche, de faire exécuter un provisioner arbitraire par le pipeline CI au prochain `terraform apply` — avec ses identifiants cloud. Un angle « chaîne d'approvisionnement » distinct de `terraform-state-exposed` (fuite passive d'un fichier déjà généré) et de `dependency-confusion-pip` (écosystème Python plutôt qu'IaC)
- **Politique de ressource cloud trop permissive** : un secret Secrets Manager dont la politique de ressource autorise `\"Principal\": \"*\"` est lisible depuis n'importe quel compte cloud, sans jamais avoir eu besoin d'assumer un rôle IAM dans le compte cible — distinct de `iam-role-overpermissive` (politique de confiance d'un rôle) car ici la faille porte sur la ressource elle-même, pas sur un rôle
- Ces deux scénarios rejoignent la famille « Cloud & Infrastructure as Code », qui passe ainsi de 9 à 11 scénarios
- Aucune nouvelle famille technique nécessaire ; le mentor contextuel et le Bilan de compétences héritent automatiquement des deux nouveaux scénarios via la famille existante

### v4.1 — Pack « conteneurs », 3ᵉ vague — secret en clair via pods/exec & API Docker distante 🟡
**Statut : fait** (2 scénarios supplémentaires, portant le total à 63)
- **Secret en clair exposé via pods/exec** : un mot de passe injecté directement comme variable d'environnement, plutôt que monté comme fichier depuis un Secret, se lit avec le seul droit `pods/exec` — bien moindre que le droit de lister les Secrets eux-mêmes. Un rappel que `pods/exec` doit être traité comme un droit sensible, pas comme un simple outil de dépannage
- **API Docker distante non authentifiée** : le démon Docker écoutant sur TCP/2375 sans authentification TLS, n'importe qui sur le réseau peut monter l'intégralité du système de fichiers hôte dans un nouveau conteneur — un accès root distant instantané sur l'hôte, distinct du socket Unix local déjà couvert par `docker-socket-writable`
- Ces deux scénarios rejoignent la famille « Conteneurs & orchestration », qui passe ainsi de 8 à 10 scénarios
- Aucune nouvelle famille technique nécessaire ; le mentor contextuel et le Bilan de compétences héritent automatiquement des deux nouveaux scénarios via la famille existante

### v4.2 — Pack « réseau », 3ᵉ vague — SNMP par défaut & module rsync anonyme 🔵
**Statut : fait** (2 scénarios supplémentaires, portant le total à 65)
- **Chaîne de communauté SNMP par défaut** : un routeur core répondant encore à `public` en SNMPv2c laisse interroger l'intégralité de sa configuration, y compris une OID propriétaire du constructeur révélant en clair un secret partagé RADIUS — un rappel que SNMPv2c ne authentifie jamais l'appelant individuellement, contrairement à SNMPv3
- **Module rsync anonyme** : un module rsync sans `auth users` expose l'intégralité d'un répertoire de sauvegarde en lecture libre, y compris une clé privée SSH de déploiement — un vecteur distinct des partages NFS/SMB déjà couverts, sur un protocole différent
- Ces deux scénarios rejoignent la famille « Réseau & annuaires », qui passe ainsi de 9 à 11 scénarios (à égalité avec Applications web et Cloud & IaC, juste derrière Linux)
- Aucune nouvelle famille technique nécessaire ; le mentor contextuel et le Bilan de compétences héritent automatiquement des deux nouveaux scénarios via la famille existante

### v4.3 — Pack « Active Directory », 4ᵉ vague — délégation contrainte S4U2Proxy & GPP cpassword 🔴
**Statut : fait** (2 scénarios supplémentaires, portant le total à 67)
- **Abus de délégation contrainte (S4U2Proxy)** : un compte de service autorisé à déléguer vers un seul SPN précis peut, via S4U2Self, usurper n'importe quel utilisateur — y compris l'administrateur — sans jamais connaître son mot de passe, puis échanger ce ticket contre un accès complet au service autorisé. La restriction ne protège que le service ciblé, jamais l'identité usurpée ; seul le marqueur « sensible et non délégable » sur les comptes à privilèges ferme cette voie
- **Mot de passe GPP (cpassword) dans SYSVOL** : une ancienne stratégie de groupe ayant poussé un mot de passe administrateur local laisse un fichier `Groups.xml` lisible par tout utilisateur du domaine dans SYSVOL — son attribut `cpassword` se déchiffre instantanément avec la clé AES fixe que Microsoft a publiée en 2012 (MS14-025), le chiffrement ne reposant sur aucun secret propre au domaine
- Ces deux scénarios rejoignent la famille « Active Directory / Windows », qui passe ainsi de 9 à 11 scénarios (à égalité avec les autres grandes familles, juste derrière Linux)
- Aucune nouvelle famille technique nécessaire ; le mentor contextuel et le Bilan de compétences héritent automatiquement des deux nouveaux scénarios via la famille existante

### v4.4 — Fiches techniques imprimables par scénario 📄
**Statut : fait**
- Nouveau bouton « 📄 Fiche technique » sur l'écran de briefing, ouvrant une page HTML autonome (imprimable / exportable en PDF via le navigateur) pour le scénario en cours
- Chaque fiche rattache la faille à une **référence réelle** — CVE, technique du référentiel MITRE ATT&CK, outil offensif connu (Responder, Impacket, mimikatz, BloodHound...) ou incident documenté publiquement (Capital One/IMDS, Log4Shell, PwnKit, dependency confusion d'Alex Birsan, MS14-025...) — avec une sévérité indicative, la méthode d'attaque et la remédiation
- Nouvelle table `js/factsheets.js`, volontairement séparée de `scenarios.js` : une entrée de métadonnées (sévérité + référence + note) par scénario, sans dupliquer ni risquer de modifier la moindre règle d'exploit ou de correctif déjà testée. Le contenu de la fiche (description, indices, correctif) est entièrement réutilisé depuis les données existantes du scénario, jamais dupliqué en dur
- Repli gracieux si une entrée de métadonnées venait à manquer (sévérité par défaut, champ référence vide) — aucun scénario ne peut faire planter la génération de sa fiche
- Vérification programmatique avant intégration : correspondance stricte 1 pour 1 entre les 67 scénarios et les 67 entrées de `FACTSHEET_META` (aucun id manquant, aucune entrée orpheline), génération de la fiche HTML testée sans erreur pour l'ensemble des 67 scénarios, et extraction du flag de validation vérifiée (avec repli explicite pour les scénarios historiques qui valident par état du terminal plutôt que par chaîne `FLAG{...}`)
- `js/factsheets.js` ajouté au cache du service worker ; cache incrémenté (v33 → v34)

---

### v4.5 — Vue diff avant/après correctif 🔍
**Statut : fait**
- En phase de défense, `verify` et `replay` (ainsi que la bannière automatique quand la configuration est détectée comme durcie) affichent désormais un panneau repliable « 🔍 Voir ce que vous avez changé », listant chaque fichier de configuration modifié avec un diff ligne à ligne façon `git diff` — lignes supprimées en rouge barré, lignes ajoutées en vert, contexte inchangé en gris
- Aucune donnée supplémentaire nécessaire dans `scenarios.js` : l'état « avant » est obtenu en rappelant `scn.makeVfs()` (pure et déterministe par construction), comparé au `game.vfs` courant — donc compatible nativement avec les 67 scénarios existants sans la moindre modification de leurs objets
- Algorithme de diff par plus longue sous-séquence commune (LCS), implémenté dans un nouveau module dédié `js/diffview.js`, entièrement séparé du moteur de jeu et des scénarios
- Le panneau ne s'affiche que s'il y a effectivement une modification à montrer (aucun panneau vide) et reste utile même quand la faille n'est *pas encore* corrigée : voir ce qu'on a déjà changé aide à repérer ce qu'il reste à faire
- Vérification programmatique avant intégration : diff testé sans erreur sur les 67 scénarios (état initial → aucune modification détectée ; état modifié → panneau généré avec lignes ajoutées/supprimées), contrôle d'intégrité croisé habituel toujours vérifié
- `js/diffview.js` chargé dans `index.html` et ajouté au cache du service worker ; cache incrémenté (v34 → v35)

---

### v4.6 — Recherche & filtres dans l'onglet Dossiers 🟡
**Statut : fait**
- Nouvelle barre au-dessus de la grille de dossiers : un champ de recherche texte libre (titre, catégorie, ou référence réelle de la fiche technique — CVE, outil, incident), un menu déroulant par **famille technique** (les mêmes 7 familles que la topologie réseau v0.6 et le Bilan v2.4) et un menu déroulant par **statut** (verrouillé / en cours / sécurisé), plus un bouton de réinitialisation
- Devenu nécessaire à cette échelle : parcourir 67 cartes une par une pour retrouver « le scénario Kerberoasting » ou « tout ce qui reste à sécuriser côté Cloud & IaC » n'avait plus de sens sans un minimum de filtrage
- Compteur « X / 67 dossiers affichés » et message dédié si aucun dossier ne correspond (plutôt qu'une grille silencieusement vide)
- Recherche insensible à la casse, appliquée en direct (léger anti-rebond de 120 ms) ; les menus famille/statut s'appliquent immédiatement au changement
- Techniquement : le rendu de la grille est extrait de `renderHome()` vers une fonction dédiée `renderMissionGrid()`, appelée à la fois au chargement de l'accueil et à chaque changement de filtre — sans dupliquer la logique de carte (statut, déblocage, boutons attaque/défense inchangés à l'identique). Les options du menu « famille » sont générées depuis `NETWORK_CLUSTERS` (v0.6, aucune duplication de la liste des familles) et la recherche par référence reprend `FACTSHEET_META` (v4.4) sans dupliquer son contenu non plus
- Filtres tenus en mémoire de session uniquement (pas de `localStorage`) : c'est une préférence d'affichage ponctuelle, pas une donnée de progression à conserver — cohérent avec le choix déjà fait pour d'autres réglages transitoires du projet
- Aucun impact sur la progression, le classement, le bac à sable ou tout autre mode : seule la fonction d'affichage de la grille de l'onglet Dossiers est concernée : `openBriefing(i, ...)` continue de recevoir l'index réel du scénario dans `SCENARIOS`, jamais un index recalculé sur la liste filtrée
- Vérification programmatique avant intégration : rendu initial (67/67), recherche texte, filtre par famille, filtre par statut et cas « aucun résultat » testés sans erreur dans un DOM headless reproduisant le chargement réel des scripts du projet

---

### v4.7 — Autocomplétion Tab dans le terminal 🟡
**Statut : fait**
- La touche **Tab** complète désormais soit un nom de commande (premier mot de la ligne), soit un chemin de fichier/dossier du système simulé (arguments suivants) — comme un vrai shell : un dossier se complète avec un `/` final (sans espace, pour continuer à taper dedans), un fichier ou une commande unique se complète avec un espace final
- En cas d'ambiguïté, Tab complète d'abord jusqu'au plus long préfixe commun aux candidats (ex. `redis-c` + Tab → `redis-cli` s'il est seul à commencer ainsi) ; si aucune progression n'est plus possible, un **second Tab** consécutif (sans autre touche entre-temps) affiche la liste des candidats dans le terminal, sur le même principe qu'un vrai shell Unix
- La complétion de chemin respecte les permissions déjà simulées : un dossier illisible (`canRead` refusé) ne propose aucun candidat, exactement comme `ls`/`cd` le refuseraient
- Techniquement : nouveau module dédié `js/autocomplete.js`, en lecture seule sur l'état du jeu (`game.vfs`/`game.cwd`/`game.user`/`game.isRoot`) — aucune règle d'exploit ni aucune donnée de scénario n'est modifiée. Le dictionnaire de commandes complétables est une liste statique combinant les commandes génériques de `dispatchBuiltin` (engine.js) et les verbes spécifiques extraits des `pattern` de `exploitRules` à travers tous les scénarios et chaînes — sans dépendance dynamique au scénario en cours, donc sans risque de désynchronisation avec les règles d'exploit, qui gardent la priorité absolue dans `runCommand()`
- Limite assumée : la complétion s'applique au dernier mot de la ligne, la position du curseur dans le champ n'est pas prise en compte — cohérent avec le reste de l'interpréteur, déjà volontairement simplifié par rapport à un vrai shell POSIX
- Effet de bord assumé : Tab est intercepté dans le champ de commande du terminal (comme dans un vrai terminal), donc il ne navigue plus vers les autres boutons de l'écran de jeu depuis ce champ — un compromis nécessaire pour un comportement de complétion crédible, à garder en tête pour un futur passage accessibilité clavier complet
- Vérification programmatique avant intégration : complétion de commande unique, complétion de dossier/fichier unique (avec et sans chemin relatif `../..`), préfixe commun partiel, liste affichée au second Tab, et remise à zéro de l'état de double-tab dès qu'une autre touche est pressée — tous testés sans erreur dans un DOM headless rejouant le chargement réel des scripts
- `js/autocomplete.js` ajouté au cache du service worker ; cache incrémenté (v35 → v36)

---

### v4.8 — Glossaire pédagogique 🟡
**Statut : fait**
- Nouveau panneau repliable **« 📖 Glossaire »** dans l'onglet Apprendre, sous forme d'un élément `<details>` natif (accessible au clavier et aux lecteurs d'écran sans JavaScript additionnel pour l'ouverture/fermeture)
- **47 termes** couvrant l'ensemble des familles techniques du jeu — permissions Linux, Active Directory, cloud/IaC, conteneurs & orchestration, réseau, applicatif web — chacun avec une définition de deux à trois phrases, indépendante de tout scénario précis (contrairement aux fiches techniques v4.4, qui elles restent spécifiques à un dossier)
- Recherche texte libre (terme et définition), avec compteur « X / 47 termes », sur le même patron visuel que le filtre de l'onglet Dossiers (v4.6) pour une cohérence d'interface
- Objectif : donner une référence transverse consultable à tout moment (y compris pendant une attaque, sans quitter l'onglet Apprendre), plutôt que de devoir rouvrir une leçon complète pour se rappeler ce que signifie un sigle croisé dans un autre scénario
- Techniquement : nouveau module autonome `js/glossary.js` (données + rendu + filtre), sans aucune dépendance sur `SCENARIOS`, `progress` ou tout autre état de jeu — un simple ajout, strictement additif, qui ne modifie ni ne lit la mécanique existante
- Vérification programmatique avant intégration : rendu des 47 termes, filtrage par recherche, absence de doublon de terme, ouverture du panneau `<details>` — testés sans erreur dans un DOM headless rejouant le chargement réel des scripts
- `js/glossary.js` ajouté au cache du service worker ; cache incrémenté (v36 → v37)

---

### v4.9 — Accessibilité clavier & ARIA 🟡
**Statut : fait**
- **Modales (piège de focus + Échap)** : les quatre fonctions qui créent une modale (`openNanoEditor`, `showModal`, `showRunCompleteModal`, `openLesson`, toutes dans `ui.js`) reçoivent désormais automatiquement — sans être modifiées elles-mêmes — un rôle `dialog`/`aria-modal`, un intitulé accessible (`aria-labelledby`), le focus posé sur le premier élément interactif à l'ouverture, un piège du Tab/Maj+Tab qui garde le focus dans la modale, et la touche **Échap** qui déclenche un clic réel sur le bouton de fermeture non destructeur (jamais un bouton d'action primaire). À la fermeture, le focus revient sur l'élément qui l'avait avant l'ouverture ; si celui-ci a disparu entre-temps (la grille de leçons se reconstruit avant même l'ouverture de la modale de leçon), repli sur le titre de la section active plutôt que de perdre le focus silencieusement sur `<body>`
- **Technique** : nouveau module `js/a11y.js`, qui observe simplement l'apparition/disparition de tout `.modal-overlay` dans `#modal-root` via `MutationObserver` — zéro modification des quatre fonctions qui créent ces modales, donc zéro risque de régression sur leur logique propre
- **Cartes de leçon** (onglet Apprendre) : jusqu'ici cliquables à la souris uniquement, elles reçoivent `role="button"`, `tabindex="0"` et la gestion Entrée/Espace — sur le même principe que les nœuds de la topologie réseau (déjà accessibles depuis la v0.6)
- **Navigation des onglets** : l'onglet actif reçoit désormais `aria-current="true"` (mis à jour dans `switchHomeTab()`), pour que les lecteurs d'écran annoncent la section courante — choix délibérément plus simple qu'un motif ARIA `tablist`/`tab` complet avec navigation aux flèches, qui aurait exigé une réécriture plus large pour rester conforme (un rôle `tab` à moitié implémenté nuit davantage qu'il n'aide)
- **Terminal** : sa sortie (`#term-body`) devient une région `role="log"`/`aria-live="polite"`, pour que les nouvelles lignes soient annoncées ; le champ de saisie reçoit un `aria-label` explicite (un simple `placeholder` n'est pas fiable comme équivalent d'étiquette) et un indicateur de focus visible (`box-shadow`), qui n'existait pas du tout auparavant
- **Correctif du compromis introduit en v4.7** : Tab reste intercepté pour l'autocomplétion, mais **Maj+Tab** est désormais volontairement laissé libre, ce qui permet à nouveau de sortir du champ terminal au clavier vers l'arrière — limite restante assumée et documentée : Tab (sans Maj) ne peut plus faire avancer le focus depuis ce champ, un vrai shell n'ayant de toute façon pas cet usage
- **Hors périmètre, assumé** : les overlays propres à `duel.html` (compte à rebours, résultat) restent inchangés — cette page n'est qu'un contrôleur embarquant deux `<iframe>` pointant chacune vers `index.html`, qui héritent donc déjà de tout ce qui précède ; ses deux overlays à elle sont de simples affichages non focusables, sans formulaire ni bouton à piéger
- Vérification programmatique avant intégration : rôle/aria-modal/aria-labelledby corrects, focus initial posé, cycle Tab et Maj+Tab dans la modale, Échap déclenchant le bon bouton, restauration exacte du focus (cas nano) et repli sur le titre de section (cas leçon, déclencheur détruit entre-temps), `aria-current` synchronisé, carte de leçon ouvrable au clavier, Maj+Tab toujours libre dans le terminal — tous testés sans erreur dans un DOM headless rejouant le chargement réel des scripts
- `js/a11y.js` ajouté au cache du service worker ; cache incrémenté (v37 → v38)

---

### v5.0 — Mode Examen chronométré 🟢
**Statut : fait**
- **Trois formats prédéfinis** : rapide (5 systèmes / 20 min), standard (8 systèmes / 35 min), complet (12 systèmes / 55 min) — un temps par système volontairement généreux (~4-4,5 min) pour laisser la place aux deux phases plutôt que de transformer l'exercice en pur chrono-speedrun
- **Tirage sans remise** parmi les 67 scénarios (Fisher-Yates), puis chaque système est traité **entièrement** — attaque *puis* défense sur le même système — avant de passer au suivant, comme le parcours principal mais enchaîné sans repasser par l'écran d'accueil et sous un chrono global qui ne s'arrête jamais entre deux systèmes
- **Nouveau module dédié `js/exam.js`**, isolé sur le même principe que le bac à sable (v0.7) ou la faille du jour (v1.2) : nouveau champ d'état `game.exam` (jamais `progress`), garde-fou `game.examPhaseDone` contre le double déclenchement (identique à `sandboxWon`/`dailyDone`), deux branches dédiées dans `checkAutoWin()` (`engine.js`) pour l'attaque et la défense
- **Décompte visible** dans le bandeau du jeu (`#hud-exam-timer`, mis à jour chaque seconde), qui passe en rouge clignotant sous la barre des 60 dernières secondes ; à zéro, la session se termine automatiquement (`endExam(true)`) même en plein milieu d'un système
- **Historique local** (jusqu'à 20 sessions, `localStorage`) : date, format, systèmes résolus, score cumulé, durée, statut (dans les temps / hors délai) — consultable dans le nouvel onglet **⏱️ Examen** de l'écran d'accueil, avec le meilleur score mis en avant
- **Deux nouveaux succès** : « Candidat » (terminer une première session, quel que soit le résultat) et « Major de promotion » (tous les systèmes traités, attaque + défense, avant l'expiration du temps)
- **Filet de sécurité** dans `goHome()` : si l'on quitte l'écran de jeu en pleine session via le logo ou le bouton Menu plutôt que par le bouton dédié « Abandonner l'examen », le chrono d'examen est arrêté proprement et `game.exam` remis à `null` — sans ce filet, relancer une mission normale depuis l'accueil aurait pu, à tort, être traitée comme une étape d'examen par `checkAutoWin()`
- **Hors périmètre, assumé** : pas de réglage libre du nombre de systèmes ou du temps (seulement les 3 formats prédéfinis) — cohérent avec la philosophie « choix simples plutôt que configuration exhaustive » déjà appliquée à la faille du jour et au bac à sable
- Vérification programmatique avant intégration : les 67 scénarios se relient tous correctement (`SCENARIOS[idx]` valide sur tout l'intervalle du tirage), enchaînement attaque→défense→système suivant sans repasser par `progress`, fin d'examen déclenchée aussi bien par la dernière défense que par l'expiration du chrono, historique correctement tronqué à 20 entrées, sortie propre de l'écran de jeu sans laisser d'intervalle actif en tâche de fond
- `js/exam.js` ajouté au cache du service worker ; cache incrémenté (v38 → v39)

---

### v5.1 — Pack « Active Directory », 5ᵉ vague — ADCS ESC1 & Zerologon 🔴
**Statut : fait** (2 scénarios supplémentaires, portant le total à 69)
- **Abus ADCS ESC1** : un modèle de certificat mal configuré (`ENROLLEE_SUPPLIES_SUBJECT` activé, aucune approbation manuelle requise, inscription ouverte à tous) permettant à n'importe quel utilisateur authentifié de demander un certificat en spécifiant l'UPN d'un autre compte — y compris `administrator` — pour s'authentifier à sa place via PKINIT ; la technique la plus courante identifiée par la recherche SpecterOps de 2021 sur Active Directory Certificate Services et popularisée depuis par l'outil Certipy
- **Zerologon (CVE-2020-1472)** : une faille cryptographique du protocole Netlogon (implémentation AES-CFB8) permettant, sans le moindre identifiant, de réinitialiser à distance le mot de passe du compte machine du contrôleur de domaine lui-même, puis d'en abuser pour une réplication DCSync complète — publiée par Secura en août 2020, notée 10.0/10 sur l'échelle CVSS
- Les deux scénarios suivent le même principe narratif que les autres du pack AD : une configuration lisible dans un fichier JSON simulé (`/etc/ad/pki/UserAuthTemplate.json`, `/etc/ad/policy/netlogon.json`), une chaîne de commandes simulées (`certipy`, `zerologon-scan`/`zerologon-exploit`, `secretsdump`) qui valident les unes après les autres l'état de cette configuration, et un correctif qui consiste à corriger l'attribut fautif plutôt qu'à supprimer la fonctionnalité entière
- Intégrés à la famille technique « Active Directory / Windows » déjà existante (topologie réseau v0.6, Bilan de compétences v2.4, filtre par famille de l'onglet Dossiers v4.6) — désormais 13 scénarios pour cette famille — et à la banque de fiches techniques (v4.4)
- Aucune modification de la banque de conseils du mentor contextuel (v2.2) : les conseils génériques déjà écrits pour la famille Active Directory (« quel attribut de configuration mal réglé », « quelle relation de confiance entre objets du domaine ») s'appliquaient déjà correctement à ces deux nouvelles techniques sans rien perdre de leur caractère socratique
- Vérification programmatique avant intégration : les deux scénarios rejoués intégralement hors DOM (`vm` Node), attaque et défense, avec capture du flag et vérification que `replay` échoue bien après correctif et réussit bien avant — plus un contrôle croisé automatisé confirmant que les 69 scénarios sont chacun présents dans `NETWORK_CLUSTERS`, `SKILL_FAMILIES` et `FACTSHEET_META`, sans id manquant ni orphelin

---

### v5.2 — Pack « Applications web », 3ᵉ vague — CSRF & XXE 🔴
**Statut : fait** (2 scénarios supplémentaires, portant le total à 71)
- **Absence de jeton CSRF** : l'endpoint de changement de mot de passe accepte toute requête authentifiée par le seul cookie de session (`SameSite=None`, aucun jeton CSRF vérifié) — un formulaire caché auto-soumis sur un site tiers suffit à modifier le mot de passe d'une victime déjà connectée, sans la moindre interaction visible de sa part
- **XXE (entité externe XML)** : le parseur XML d'un endpoint d'import ne désactive pas la résolution des entités externes déclarées en DTD — un document XML forgé avec une entité `SYSTEM` ciblant un fichier local (`file:///etc/api/secrets.env`) suffit à en exfiltrer le contenu directement dans la réponse
- Ces deux techniques comblent une lacune de couverture identifiée dans l'audit des familles existantes (le pack « Applications web » couvrait déjà JWT, désérialisation, SSTI, IDOR, mass assignment, exposition de données, limitation de débit, GraphQL et CORS, mais ni CSRF ni XXE — deux classiques historiques du OWASP Top 10)
- Intégrés à la famille technique « Applications web » déjà existante (topologie réseau v0.6, Bilan de compétences v2.4 sous le nom « Web & API », filtre par famille de l'onglet Dossiers v4.6) — désormais 13 scénarios pour cette famille — et à la banque de fiches techniques (v4.4)
- Vérification programmatique avant intégration : les deux scénarios rejoués intégralement hors DOM (`vm` Node) comme pour le pack v5.1, plus le même contrôle croisé automatisé étendu aux 71 scénarios (`NETWORK_CLUSTERS`, `SKILL_FAMILIES`, `FACTSHEET_META` : aucun id manquant ni orphelin)

---

### v5.3 — Nouvelle famille « Mobile & API embarquées » 🆕
**Statut : fait** (3 scénarios, portant le total à 74 — 7ᵉ famille technique du projet)
- **Clé API codée en dur dans un APK décompilé** : une clé embarquée dans le code source décompilé de l'application mobile, censée ne servir qu'aux appels publics, est en réalité acceptée telle quelle par un endpoint d'administration du backend — l'anti-pattern classique d'une portée (scope) non distinguée entre clé mobile et accès interne
- **Absence de certificate pinning** : l'application fait confiance à n'importe quelle autorité de certification du système sans jamais vérifier le certificat précis attendu, permettant l'interception du trafic HTTPS (jeton de session compris) par un attaquant positionné sur le même réseau
- **redirect_uri OAuth non validé** : le serveur d'autorisation accepte n'importe quel `redirect_uri` fourni dans la requête au lieu de le comparer à une valeur exacte enregistrée pour l'application, permettant de rediriger le jeton d'accès d'une victime vers un domaine attaquant
- Première nouvelle famille technique ajoutée depuis le lancement du projet : nouvelle entrée `Mobile & API embarquées` dans `NETWORK_CLUSTERS` (topologie réseau v0.6) et `SKILL_FAMILIES` (Bilan de compétences v2.4, désormais un radar à 7 axes au lieu de 6 — le nombre d'axes du graphique radar SVG était déjà calculé dynamiquement à partir de `SKILL_FAMILIES.length`, donc aucune modification de rendu nécessaire)
- Nouvelle banque de conseils du mentor contextuel (v2.2) dédiée à cette famille, plutôt que de laisser reposer les trois scénarios sur le mentor générique de repli
- 3 nouveaux termes ajoutés au glossaire pédagogique (v4.8) : certificate pinning, MASVS, redirect_uri — portant le glossaire à 50 termes
- Vérification programmatique avant intégration : les trois scénarios rejoués intégralement hors DOM (`vm` Node), et le même contrôle croisé automatisé étendu aux 74 scénarios au total

---

### v5.4 — Script de test formalisé (`test/validate-scenarios.js`) 🧪
**Statut : fait**
- Les vérifications ad hoc refaites à la main dans `/tmp` à chaque ajout de scénario depuis la v5.1 (cohérence des ids, structure du système de fichiers simulé, invariant d'état initial, résolution via les indices) sont désormais un vrai script versionné, exécutable avec `node test/validate-scenarios.js` ou `npm test` — zéro dépendance, cohérent avec la philosophie « aucun build, aucun paquet » du reste du projet
- **Quatre familles de vérifications** : (1) cohérence des ids entre `SCENARIOS`, `NETWORK_CLUSTERS`, `SKILL_FAMILIES` et `FACTSHEET_META`, dans les deux sens (absence *et* orphelins) ; (2) structure individuelle de chaque scénario (titre, phases, fonctions de contrôle, cohérence complète du système de fichiers simulé — chaque dossier référence des enfants qui existent, chaque chemin est bien listé par son parent) ; (3) invariant d'état initial : un `makeVfs()` fraîchement généré ne doit jamais être déjà « corrigé » (`defenseCheck` faux, `replay` réussi, `attackCheck` faux avant toute action) ; (4) solveur best-effort qui extrait les commandes citées entre backticks dans les indices d'attaque, les rejoue contre les `exploitRules`, et vérifie que `attackCheck` devient vrai — exactement le chemin qu'emprunterait un joueur suivant les indices à la lettre
- Distinction délibérée entre échec critique (code de sortie 1 : structure cassée, id incohérent, invariant violé) et avertissement (le solveur automatique n'a pas pu résoudre un scénario à partir de ses indices — souvent un scénario plus ancien qui s'appuie sur `nano`/l'éditeur interactif plutôt que sur des `exploitRules` à commande unique, pas nécessairement un bug)
- Corrige au passage un vrai bug latent découvert par le script lui-même en première exécution : la vérification `pattern instanceof RegExp` échouait systématiquement pour tous les scénarios, non pas à cause d'un défaut des scénarios mais d'un piège classique des sandbox `vm` de Node — une expression régulière construite dans un contexte `vm` n'est pas une instance du `RegExp` de l'hôte (deux réalms différents) ; remplacé par `Object.prototype.toString.call(...)`, sûr entre réalms
- Ne remplace pas une relecture humaine ni un test en navigateur réel : le script documente explicitement, dans son en-tête, ce qu'il vérifie et ce qu'il NE vérifie PAS (rendu visuel, accessibilité, correspondance exacte entre `defense.hints` et `defenseCheck`)
...
*Dernière mise à jour : implémentation de v0.4 (scoring, chrono, classement local), v0.5 (rapport de session Markdown), v0.6 (topologie réseau interactive), v0.7 (bac à sable), v0.8 (ambiance sonore), v0.9 (mode face-à-face local par iframes synchronisées), v1.0 (chaîne à mouvement latéral sur 3 machines distinctes), v1.1 (thèmes clair/contraste élevé), v1.2 (faille du jour avec série façon Wordle, puis complétée cette session par un classement simulé localement), v1.3 (récap cinématique + export HTML autonome), v2.0 (génération procédurale à 2 modèles + éditeur de scénario sans code), v2.1 (difficulté adaptative : série sans indice + mode guidé progressivement moins bavard, puis complétée par des commandes gratuites variables selon la série), v2.2 (mentor contextuel : conseils socratiques par famille technique, puis enrichis de 3 à 5 conseils par phase), v2.3 (pack « identité & secrets cloud » — total 35 scénarios), v2.4 (Bilan : temps moyen et taux d'indices par famille, courbe de score chronologique, plus deux correctifs d'intégrité de la progression sauvegardée), v2.5 (mode revanche : repérage des points faibles par score combiné, relance ciblée en bac à sable pour l'attaque *et* pour la défense) et v2.6 (pack « Active Directory / Windows » — AS-REP Roasting, délégation sans contrainte, abus DCSync, GPO modifiable — total 39 scénarios, nouvelle famille technique dédiée) et v2.7 (pack « sécurité des API » — IDOR sur une API de facturation, affectation de masse à l'inscription — total 41 scénarios, famille « Applications web » existante) et v2.8 (pack « sécurité des API », 2ᵉ vague — exposition excessive de données, absence de limitation de débit — total 43 scénarios) et v2.9 (RBAC Kubernetes trop permissif, confusion de dépendances pip — total 45 scénarios) et v3.0 (pack « réseau » — Memcached non authentifié, session nulle SMB — total 47 scénarios) et v3.1 (polish visuel : apparition échelonnée des cartes dossier, liseré de couleur par famille technique) et v3.2 (structuration visuelle de la nav : icônes sur tous les onglets, séparateurs de groupe) et v3.3 (célébration cinématique à la capture d'un flag : flash plein écran, secousse du terminal, texte pulsé) et v3.4 (pack « conteneurs » — PID host partagé, NetworkPolicy absente — total 49 scénarios) et v3.5 (pack « Active Directory » — Kerberoasting, Pass-the-Hash — total 51 scénarios, famille AD/Windows portée de 5 à 7) et v3.6 (pack « conteneurs », 2ᵉ vague — etcd non authentifié, évasion cgroup release_agent — total 53 scénarios, famille Conteneurs portée de 6 à 8) et v3.7 (pack « web », 2ᵉ vague — introspection GraphQL, CORS reflété — total 55 scénarios, famille Applications web portée de 9 à 11) et v3.8 (pack « réseau », 2ᵉ vague — empoisonnement LLMNR/NBT-NS, relais NTLM faute de signature SMB — total 57 scénarios, famille Réseau & annuaires portée de 7 à 9) et v3.9 (pack « Active Directory », 3ᵉ vague — Silver Ticket, abus d'ACL GenericAll — total 59 scénarios, famille AD/Windows portée de 7 à 9) et v4.0 (pack « cloud », 2ᵉ vague — chaîne d'approvisionnement Terraform non épinglée, politique de ressource cloud publique — total 61 scénarios, famille Cloud & IaC portée de 9 à 11) et v4.1 (pack « conteneurs », 3ᵉ vague — secret en clair via pods/exec, API Docker distante non authentifiée — total 63 scénarios, famille Conteneurs portée de 8 à 10) et v4.2 (pack « réseau », 3ᵉ vague — SNMP par défaut, module rsync anonyme — total 65 scénarios, famille Réseau & annuaires portée de 9 à 11) et v4.3 (pack « Active Directory », 4ᵉ vague — délégation contrainte S4U2Proxy, GPP cpassword — total 67 scénarios, famille AD/Windows portée de 9 à 11) et v4.4 (fiches techniques imprimables par scénario, rattachant chaque faille à une référence réelle — CVE, technique MITRE ATT&CK, incident documenté) et v4.5 (vue diff avant/après correctif : panneau repliable montrant ligne à ligne ce que le joueur a changé dans la configuration, en réutilisant `scn.makeVfs()` comme état de référence) et v4.6 (recherche & filtres dans l'onglet Dossiers : texte libre, famille technique, statut, avec compteur et état vide dédié) et v4.7 (autocomplétion Tab dans le terminal : commandes et chemins, préfixe commun, liste des candidats au second Tab) et v4.8 (glossaire pédagogique transverse : 47 termes, recherche, panneau repliable natif dans l'onglet Apprendre) et v4.9 (accessibilité clavier & ARIA : piège de focus et Échap génériques pour toutes les modales, cartes de leçon accessibles au clavier, aria-current sur l'onglet actif, terminal en région live, correctif Maj+Tab) et v5.0 (mode Examen chronométré : trois formats prédéfinis, tirage sans remise, chrono global visible au HUD, historique local des sessions, deux nouveaux succès) et v5.1 (pack « Active Directory », 5ᵉ vague — abus ADCS ESC1, Zerologon CVE-2020-1472 — total 69 scénarios, famille AD/Windows portée de 11 à 13) et v5.2 (pack « Applications web », 3ᵉ vague — absence de jeton CSRF, XXE — total 71 scénarios, famille Applications web portée de 11 à 13) et v5.3 (nouvelle famille « Mobile & API embarquées », 7ᵉ famille technique du projet — clé API codée en dur, absence de certificate pinning, redirect_uri OAuth non validé — total 74 scénarios) et v5.4 (script de test formalisé `test/validate-scenarios.js`, exécutable via `npm test`, qui a d'ailleurs révélé et corrigé un piège classique des sandbox `vm` de Node lors de sa toute première exécution), plus un système de succès non planifié initialement — sans modifier la mécanique des scénarios existants. À cette occasion, la timeline condensée de la page d'accueil (`js/hero-fx.js`) a également été resynchronisée avec ce fichier détaillé.*
